...
{
    /* Enable UART0 (Console stdout log) */
    ...
    /* Enable UART1 (Dedicated CAN SLCAN streaming channel) */
...
void Can_SendSLCAN(const char* str)
{
    while (*str) {
        UARTCharPut(UART1_BASE, (unsigned char)*str++);
    }
}
