...
| **10** | [10_CAN_DBC_Format_And_Tools.md](file:///C:/Users/liem.vu/Liem.vuOD/Study_AUTOSAR-main/docs/theory/10_CAN_DBC_Format_And_Tools.md) | **9.0 / 10** | • **Ma Trận Gói Tin CAN**: Cấu trúc Vector DBC, Signal Encoding/Decoding, Intel vs Motorola.<br>• **Case Study Thực Tế**: Phân tích toàn bộ 11 gói tin CAN trong mã nguồn gốc dự án `as`.<br>• **Mã nguồn:** `as/build/nt/lm3s6965evb/ascore/config/CanIf_Cfg.c`, `Com_PbCfg.c`. |
| **11** | [11_LIN_Protocol_And_LinStack_Deep_Dive.md](file:///C:/Users/liem.vu/Liem.vuOD/Study_AUTOSAR-main/docs/theory/11_LIN_Protocol_And_LinStack_Deep_Dive.md) | **9.0 / 10** | • **Giao Thức LIN (Local Interconnect Network)**: Single Master / Multiple Slaves, Break/Sync 0x55, PID, LDF file.<br>• **Ngăn Xếp LinStack**: `Lin` (MCAL), `LinIf` (Schedule Table), `LinSM` (Mode Manager), `LinTp`.<br>• **Mã nguồn:** `communication/Lin/` (`Lin.c`, `LinIf.c`, `LinSM.c`, `LinTp.c`). |
| **12** | [12_Automotive_Ethernet_SOMEIP_DoIP_Deep_Dive.md](file:///C:/Users/liem.vu/Liem.vuOD/Study_AUTOSAR-main/docs/theory/12_Automotive_Ethernet_SOMEIP_DoIP_Deep_Dive.md) | **9.5 / 10** | • **Automotive Ethernet (100BASE-T1)**: Kiến trúc hướng dịch vụ SOA, 1 cặp dây xoắn BroadR-Reach.<br>• **Ngăn Xếp EthStack**: `Eth` (MCAL), `EthIf`, `EthSM`, `TcpIp`, `SoAd` (Socket Adaptor), `SOME/IP` & `SD`, `DoIP` (ISO 13400).<br>• **Mã nguồn:** `communication/Eth/`, `EthIf/`, `SoAd/`, `DoIP/`, `SD/`. |
